...
const childPosition = ref({ x: 0, y: 0 });
const isDragging = ref(false);
const offset = ref({ x: 0, y: 0 });
let positionFromLeftBottom= ref('');
...
// let positionFromLeftBottom = calculateDistance( parent.value.getBoundingClientRect(),
//   child.value.getBoundingClientRect());
// let obj = computed(() => {
  // positionFromLeftBottom.value= calculateDistance( parent.value.getBoundingClientRect(),
  // child.value.getBoundingClientRect());
...
//   if (!parent.value || !child.value) return { x: 0, y: 0 };
